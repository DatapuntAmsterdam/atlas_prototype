  ...
  sizes.forEach((size) => {
    ...
    describe(`Homepage checks, resolution is: ${size}`, () => {
      ...
      it('Should check the footer', () => {
        ...
        cy.get(HOMEPAGE.footerBlock).eq(0).contains('Databeleid').first().click({ force: true })
        cy.url().should('include', '/artikelen/artikel/amsterdam-en-data/')
        cy.go('back')
        cy.get(HOMEPAGE.footerBlock).eq(0).contains('Bronnen').first().click({ force: true })
        cy.url().should('include', '/artikelen/artikel/bronnen/')
        cy.go('back')
        cy.get(HOMEPAGE.footerBlock).eq(0).contains('Over deze site').first().click({ force: true })
        cy.url().should('include', '/artikelen/artikel/over-deze-site/')
        cy.go('back')
        cy.get(HOMEPAGE.footerBlock).eq(0).contains('Over OIS').first().click({ force: true })
        ...
        cy.get(HOMEPAGE.footerBlock).contains('Nieuwsbrief OIS').should('exist')
        cy.get(HOMEPAGE.footerBlock).contains('Vacatures').should('exist')
        cy.get(HOMEPAGE.footerBlock).contains('Twitter').should('exist')
        cy.get(HOMEPAGE.footerBlock).contains('Facebook').should('exist')
        cy.get(HOMEPAGE.footerBlock).contains('LinkedIn').should('exist')
        cy.get(HOMEPAGE.footerBlock).contains('GitHub').should('exist')

        cy.get(HOMEPAGE.footerBlock).find('h3').contains('Vragen')
        cy.get(HOMEPAGE.footerBlock).contains('Veelgestelde vragen').first().click({ force: true })
        cy.url().should('include', '/artikelen/artikel/veelgestelde-vragen/')
        cy.go('back')
        cy.get(HOMEPAGE.footerBlock).contains('Contact opnemen').first().click({ force: true })
        cy.url().should('include', '/artikelen/artikel/contact/')
        cy.go('back')
        cy.get(HOMEPAGE.footerBlock).contains('Feedback geven').first().click({ force: true })
        cy.get('[class*="ModalStyle__ModalFocus"]').should('be.visible')
        cy.get('[title="Sluit"]').click()
        cy.get(HOMEPAGE.footerBlock).contains('Uitleg gebruik').first().click({ force: true })
